...
#if defined(NDEBUG)
# error "Kaori cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Kaori Signed Message:\n";
...
{
    // Kaori: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   // AFTER BLOCK 1
    ...
    // AFTER BLOCK 240K -> DEFLATION
    if (nHeight > 480000)
        nSubsidy = (10000000/nHeight) * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 30 * 60; // Kaori: 30 minutos // Calculate Diff
static const int64 nTargetSpacing = 2 * 60; // Kaori: 2 minuto Block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kaori: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kaori: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        // Original Date: 30/Nov/2017 
        const char* pszTimestamp = "Kaori to moon!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xaa, 0xa2, 0x26, 0xa9 }; // Kaori: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KaoriMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KaoriMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KaoriMiner(CWallet *pwallet)
{
    printf("KaoriMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kaori-miner");
    ...
    try { loop {
        ...
        printf("Running KaoriMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KaoriMiner, pwallet));
}
